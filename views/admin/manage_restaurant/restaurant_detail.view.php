  ...
  <div class="card mx-2 stat-cards-item">
    <div class="row g-0 d-flex">
      <div class="col-md-6 d-flex">
        <img src="<?= $target_file_dir . $restaurant["restaurant_img"] ?>" class="rounded" alt="restaurant image"
          style="width: 100%; margin: auto;">
      </div>
      <div class="col-md-6 d-flex flex-column justify-content-between p-0">
        <div class="card-body d-flex flex-column justify-content-between">
          <div class="">
            <h3 class="stat-cards-info__num " style="font-size: 1.5rem;">Restaurant: <span
                class="stat-cards-info__title" style="font-size: 1.5rem;">
                <?= $restaurant["restaurant_name"] ?>
              </span></h3>
            <h5 class="stat-cards-info__num mb-3" style="font-size: 1.2rem;">Owner: <span class="stat-cards-info__title"
                style="font-size: 1rem;">
                <?= $restaurant["first_name"] ?>
              </span></h5>
            <strong class="stat-cards-info__num" style="font-size: 14px;">Contact Info: <span
                class="stat-cards-info__title">
                ...
                <div class="modal-body">
                  <div class="row form-group">
                    <div class="col-6">
                      ...
                      <input type="text" name="restaurant_name" class="form-control border"
                        value="<?= $restaurant["restaurant_name"] ?>" id="restaurant_name">
                    </div>
                        ...
                        <?php foreach ($owners as $owner) {
                          if ($owner["role_id"] == 1) { ?>
                            <option value="<?= $owner["user_id"] ?>">
                              <?= $owner["first_name"] . " " . $owner["last_name"] ?>(Admin)
                            </option>
                          <?php } else {
                            if ($owner["user_id"] == $restaurant["owner_id"]) { ?>
                              <option value="<?= $owner["user_id"] ?>" selected>
                  ...
                  <div class="row form-group">
                    <div class="col-6">
                      ...
                      <input type="email" name="email" value="<?= $restaurant["email"] ?>" class="form-control border"
                        id="email">
                    </div>
                    <div class="col-6">
                      <label for="location" class="col-form-label">Location:</label>
                      <input type="text" name="location" value="<?= $restaurant["location"] ?>"
                        class="form-control border" id="location">
                    </div>
                  </div>
                  <div class="row form-group">
                    <div class="col-6">
                      <label for="contact_info" class="col-form-label">contact Info:</label>
                      <input type="tel" name="contact_info" value="<?= $restaurant["contact_info"] ?>"
                        class="form-control border" id="contact_info">
                    </div>
                    <div class="col-6">
                      <label for="restaurant_img" class="col-form-label">Image:</label>
                      <input type="file" name="restaurant_img" value="<?= $restaurant["restaurant_img"] ?>"
                        class="form-control border" id="restaurant_img">
                    </div>
                  </div>
                  <div class="mb-3">
                    <label for="description" class="col-form-label">Message:</label>
                    <textarea name="description" class="form-control" id="description"
                      style="max-height: 100px; min-height: 50px;  "><?= $restaurant["description"] ?></textarea>
                  </div>
                </div>
